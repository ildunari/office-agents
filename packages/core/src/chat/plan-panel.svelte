      ...
      <div class="mt-2 space-y-1">
        {#each plan.steps as step (step.id)}
          <div class="flex items-start gap-2 text-xs">
            <div class={`mt-0.5 h-2 w-2 rounded-full shrink-0 ${step.status === "completed" ? "bg-green-500" : step.status === "in_progress" ? "bg-(--chat-accent)" : step.status === "blocked" || step.status === "failed" ? "bg-red-500" : "bg-(--chat-text-muted)"}`}></div>
            <div class="min-w-0">
              <div class="text-(--chat-text-primary)">{step.title}</div>
              <div class="text-(--chat-text-muted)">{step.objective}</div>
            </div>
          </div>
        {/each}
      </div>
